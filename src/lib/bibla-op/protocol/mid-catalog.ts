 ...
 *
 * Each entry describes a MID's number, name, category, direction
 * (integrator→controller or vice versa), whether it carries data,
 * its data field schema, and supported revisions.
 */

import { DIGITAL_INPUT_OPTIONS } from './protocol-constants';

/** Definition of a single MID in the catalog */
...
/** Schema for a data field within a MID definition */
...
// All known MIDs organized by category
...
/** Map for O(1) MID lookup by number */
...
/** Get all MID definitions as an array */
export function getAllMids(): MIDDefinition[] {
  return [...MID_CATALOG_ARRAY];
}

/** Build a default data field string from a MID definition's field schemas */
